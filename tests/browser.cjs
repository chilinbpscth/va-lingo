...
(async()=>{
 ...
 await page.route('**/roster.json',r=>r.fulfill({contentType:'application/json',body:JSON.stringify({students:[{class:'4A',id:'01',seat:'01',name:'PRIVATE-NAME-A'},{class:'4A',id:'02',seat:'02',name:'PRIVATE-NAME-B'}]})}));
 ...
 await page.locator('#field-class option[value="4A"]').waitFor({state:'attached'});
 await page.selectOption('#field-class','4A');await page.selectOption('#field-student','01');
 await page.evaluate(url=>localStorage.setItem('vaSubmitUrl',url), 'http://127.0.0.1:'+server.address().port+'/exec');
 await page.reload({waitUntil:'domcontentloaded'});
 await page.locator('#field-class option[value="4A"]').waitFor({state:'attached'});
 await page.selectOption('#field-class','4A');await page.selectOption('#field-student','01');
 await page.waitForFunction(()=>document.querySelector('#gallery-status').textContent.includes('已登入'));
 ...
 await page.selectOption('#field-student','02');await page.locator('.source-tab[data-source="self"]').click();
 assert.equal(await page.locator('#self-work-history').count(),0);
 assert.equal(await page.locator('#step-content .scaffold-blank').first().innerText(),'');
 await page.selectOption('#field-student','01');
 assert.match(await page.locator('#step-content .scaffold-blank').first().innerText(),/我的第一份作品/);
 ...
 await page.selectOption('#field-student','02');
