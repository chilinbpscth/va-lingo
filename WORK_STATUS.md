...
互評恢復已接 listPeerWorks.myAssessment，只回當前登入學生對該作品的評語；選作品時恢復答案及 pins，保留本機已有答案。API 測試核對評者可讀回自己評語而另一學生不見；recovery module 測試與既有 Chrome 回歸通過（21 項 Node）。仍需 Chrome 新增清空後恢復互評驗證，以及 gallery 超過 12 件時的分頁／已評作品可達性；closed 課堂尚不能讀互評，不可當完整歷程已完成。

畫廊改為每頁 6 件，手動 refresh 逐頁讀至 nextOffset=null，不再截去第 13 件以後作品；讀取期間身份／課堂變更即放棄結果。closed 課堂可讀回畫廊及自己的互評，自／互評寫入均拒絕。15 件合成作品測試驗證可讀完整 14 件同儕作品與 closed 寫入拒絕；22 項 tests、check、Chrome 通過。尚未以真 Google 驗證整班容量、延遲及跨來源登入。


Chrome 已新增並通過：清空本機後取回自己的互評答案；伺服器轉到 2027-28 登入時建立獨立草稿，保留 2026-27。準備凍結本機測試候選套件。下一實際 gate 是獲批後建立獨立測試 Google 資源並 live 驗證；目前不宣稱 MVP 完成。
